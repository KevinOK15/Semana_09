...
public class Mochila extends JFrame implements ActionListener {
	...
	void obtenerDatos(){
		marc = cboModelo.getSelectedItem().toString();
		cant = Integer.parseInt( txtCantidad.getText() );
	}
	void calcularPrecio(){
		if ( marc=="Sherman" ){ prec=149.9; }
		else if ( marc=="Faguo" ){ prec=89.9; }
		else if ( marc=="Aldo" ){ prec=119.9; }
		else { prec=174.8; }
	}
	...
	void calcularChocolates(){
		if ( impP<200 ){ choc=0; }
		else if ( impP>=200 && impP<500 ){ choc=1*cant; }
		else if ( impP>=500 && impP<700 ){ choc=2*cant; }
		else { choc=3*cant; }
	}
